<?php
	...
	$mapel_kelas = new Mapel_per_kelas;
  	$id_siswa = $_GET['id'];
  	$dataSiswa = $siswa->getDataByIdJoinKelas($id_siswa);
	$dataNilai = $mapel_kelas->getNilaiFromSiswa( $id_siswa);
	$tanggal = date('d F Y');
...
foreach ($dataNilai as $key => $value) {
	...
	$pdf->Cell(120,7,$value['mapel'],1,0,'L');
	$pdf->Cell(25,7,$value['nilai'],1,1,'C');
	$total+=$value['nilai'];
	$i++;
}
